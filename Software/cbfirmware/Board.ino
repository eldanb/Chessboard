...
void BoardConfig::loadPlayerStateFromCustomBoardPosition() {
  for(int color=0; color<2; color++) {
    int homeRow = (1-color) * 7;
    if(getPieceAt(BoardCoordinate(0, homeRow)).getPieceType() != PIECE_TYPE_ROOK) {
      _playerState[color]._flags |= PLAYER_STATE_FLAG_ROOK0_MOVED;
    }
    if(getPieceAt(BoardCoordinate(7, homeRow)).getPieceType() != PIECE_TYPE_ROOK) {
      _playerState[color]._flags |= PLAYER_STATE_FLAG_ROOK7_MOVED;
    }
    if(getPieceAt(BoardCoordinate(4, homeRow)).getPieceType() != PIECE_TYPE_KING) {
      _playerState[color]._flags |= PLAYER_STATE_FLAG_KING_MOVED;
    }
  }
}
...
bool BoardConfig::visitAllMoves(PIECE_COLOR movingColor, MoveVisitor &visitor) const {
    FilterKingExposingVisitors movesFilter(*this, movingColor, visitor);
    return visitAllImmediateMoves(movingColor, movesFilter);
}
...
bool BoardConfig::visitCastlingIfPathClear(PIECE_COLOR castlingColor, const BoardCoordinate &king, const BoardCoordinate &rook, MoveVisitor &visitor) const {
    if(getPieceAt(rook).getPieceType() != PIECE_TYPE_ROOK) {
      return true;
    }
...
MOVE_TYPE BoardConfig::applyMove(const BoardCoordinate &from, const BoardCoordinate &to) {
    ...
    if(moveType & MOVE_TYPE_CAPTURE) {
        ...
        if(removedPiece.getPieceType() == PIECE_TYPE_ROOK) {
          if( (removedPiece.getPieceColor() == PIECE_COLOR_BLACK && to.getRow() == 7) ||
              (removedPiece.getPieceColor() == PIECE_COLOR_WHITE && to.getRow() == 0) ) {
             if(to.getCol() == 0) {
                _playerState[removedPiece.getPieceColor()]._flags |= PLAYER_STATE_FLAG_ROOK0_MOVED;
             } else 
             if(to.getCol() == 7) {
                _playerState[removedPiece.getPieceColor()]._flags |= PLAYER_STATE_FLAG_ROOK7_MOVED;
             }              
          }
        }
    }
